...
def get_vjudge_dir(settings, title):
    parent_dir = settings.get("vjudge_dir", '.')
    return os.path.join(parent_dir)
...
def createFile(dir_path, title, extension, codechef_directory, spoj_directory, vjudge_directory):
    ...
    if "codechef" in last_url:
        ...
        try:
            filename = os.path.join(vjudge_directory, str(re.findall(r'[A-Z]+', last_url)[-1]) + "_" +str(re.findall(r'\d+',last_url)[-1]) + '.' + extension)
        except IndexError:
            filename = os.path.join(vjudge_directory, str(re.findall(r'[A-Z]+', last_url)[-1]) + '.' + extension)
    else:
...
def fetch(self, url):
    ...
    vjudge_directory = get_vjudge_dir(settings, title)
    file = createFile(dir_path, title, extension, codechef_directory, spoj_directory, vjudge_directory)
